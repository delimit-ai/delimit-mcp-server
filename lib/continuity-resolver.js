...
// LED-5658: these were frozen module-level constants built from bare
// os.homedir(), bypassing the LED-1188 single-source-of-truth resolver
// (lib/delimit-home.js) entirely — the ONE place in this file that didn't
// honor $DELIMIT_HOME / $DELIMIT_NAMESPACE_ROOT, and didn't re-resolve if a
// test mutated process.env after this module was first required. Now
// functions so they call delimitHome() fresh every time, consistent with
// every other path in this file (see resolveContinuityContext below).
function knownWorkspacesFile() {
    return path.join(delimitHome(), 'known_workspaces.json');
}
const WORKSPACE_CACHE_TTL_MS = 6 * 60 * 60 * 1000;
function activeVentureFile() {
    return path.join(delimitHome(), 'active_venture.json');
}
...
// Explicit chat project selection is local filesystem validation only. No
// active-venture/newest-transcript inference and no configuration writes.
function resolveChatProject(project, options = {}) {
    if (!project) return null; // preserve neutral owner/root lead behavior
    const env = { ...(options.env || process.env) };
    for (const key of ['GIT_DIR', 'GIT_WORK_TREE', 'GIT_INDEX_FILE',
        'GIT_OBJECT_DIRECTORY', 'GIT_COMMON_DIR', 'GIT_QUARANTINE_PATH']) delete env[key];
    let selected, root;
    try {
        selected = fs.realpathSync(path.resolve(options.cwd || process.cwd(), project));
        const home = fs.realpathSync(options.home || os.homedir());
        if (selected === home || !fs.statSync(selected).isDirectory()) throw new Error();
        root = fs.realpathSync(require('child_process').execFileSync('git',
            ['-C', selected, 'rev-parse', '--show-toplevel'],
            { env, encoding: 'utf8', stdio: ['ignore', 'pipe', 'pipe'] }).trim());
        if (root === home) throw new Error();
    } catch (_) {
        throw new Error('Choose an existing Git project with --project /path/to/repo; the home directory is an unscoped owner workspace.');
    }
    return { cwd: selected, repoRoot: root, venture: path.basename(root) };
}
...
function loadKnownWorkspaces() {
    const data = readJson(knownWorkspacesFile(), { workspaces: [] });
...
function saveKnownWorkspace(candidate) {
    ...
    writeJson(knownWorkspacesFile(), { workspaces: next.slice(0, 20) });
}

function loadActiveVenture() {
    return readJson(activeVentureFile(), null);
}

function saveActiveVenture(entry) {
    writeJson(activeVentureFile(), {
...
function resolveContinuityContext(options = {}) {
    // LED-5658: was `process.env.DELIMIT_HOME || path.join(os.homedir(), '.delimit')`
    // — a duplicate, less-safe copy of the LED-1188 resolver (no
    // DELIMIT_NAMESPACE_ROOT fallback, and no protection against an
    // ambient/profile-leaked DELIMIT_HOME defeating a $HOME-only isolation
    // override). This is the FIRST thing bin/delimit-cli.js calls at
    // startup (its result is immediately re-exported to
    // process.env.DELIMIT_HOME for every downstream consumer, including
    // MEMORY_DIR), so fixing delimit-home.js alone would not have been
    // enough without also routing this call through it.
    const homeDir = delimitHome();
