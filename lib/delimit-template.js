...
### Session Lifecycle
- **Session start (do this FIRST, before anything else)**: call \`delimit_revive\` and load the returned soul — the last task, key decisions, and next steps. This is how you resume after switching agents (e.g. a quota-driven Claude Code → Codex switch) without losing the plot. Then \`delimit_ledger_context\` + \`delimit_gov_health\`.
