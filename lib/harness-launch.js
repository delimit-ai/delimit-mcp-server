...
function bootstrap(cwd, home, instructionSources = [], neutralWorkspace = false) {
    ...
    if (neutralWorkspace) {
        text = `Delimit owner workspace: ${JSON.stringify(cwd)}. This is an unscoped portfolio lead session, not a venture repository or build permit. No venture or project handoff has been selected.\n`
            + 'Read applicable owner/workspace instructions. Use existing Delimit ledger and continuity tooling to establish the requested venture before venture-specific action; never infer a project from home, the newest transcript or a stale active-venture pointer. Preserve dirty edits, current authority, pause/arming and native approval controls. Switching harnesses expands no authority. Save any handoff against its actual established scope; do not invent a project-bound receipt.\n';
    }
    if (fs.existsSync(custom)) text += '\n' + fs.readFileSync(custom, 'utf8');
    if (instructionSources.length) {
        text += '\nRequired project instruction sources (local references, not their contents):\n'
            + instructionSources.map(source => JSON.stringify(source)).join('\n') + '\n'
            + 'Read every listed source completely with permitted local read tools BEFORE task execution, including CLAUDE.md when native AGENTS.md precedence hides it. Read large files in bounded chunks through EOF; do not treat a preview or summary as the full guidance. Preserve native scope/precedence and newer explicit owner overrides; a shadowed file is not permission to revive superseded instructions. If applicable instructions conflict or cannot be read, report the exact gap before making changes. Do not overwrite, delete, merge, or rewrite these files automatically. These references grant no additional tool or private-data authority.\n';
    }
...
function launchExplicitHarness(id, options = {}) {
    ...
    try {
        ...
        // Only chat opts into the existing owner-home lead workspace. Native
        // shim/default callers retain their project-only contract.
        const atHome = cwd === fs.realpathSync(env.HOME || os.homedir());
        const neutralWorkspace = options.allowOwnerWorkspace === true && atHome;
        if (!neutralWorkspace && (git.status !== 0 || !String(git.stdout || '').trim() || atHome)) {
            throw new HarnessLaunchError('Select an actual venture repository, or launch an owner workspace with delimit chat --model ' + id + '. Use delimit chat --project /path/to/repo for an explicit project.');
        }
        const root = neutralWorkspace ? cwd : fs.realpathSync(String(git.stdout).trim());
        ...
        while (true) {
            for (const name of ['AGENTS.md', 'CLAUDE.md']) {
                ...
                if (fs.existsSync(file)) {
                    ...
                    instructionSources.push({path: file, bytes: bytes.length,
        ...
        const packet = bootstrap(cwd, home, instructionSources, neutralWorkspace);
        let nativeArgs;
        if (id === 'muse') {
        ...
        console.log(`[Delimit] ${id} ${neutralWorkspace ? 'owner workspace (no venture selected)' : 'project session'}; native approvals remain active. Save a correctly scoped Delimit handoff before exit.`);
...
function installHarnessShims({delimitHome, packageRoot, includeNew = true}) {
    const dir = path.join(delimitHome, 'shims');
    const planned = [], skipped = [];
    // Preflight every selected path before any write. Default setup only
    // refreshes managed shims; adding a newly shadowed command is opt-in.
    for (const id of HARNESS_IDS) {
        const file = path.join(dir, id);
        let stat;
        try { stat = fs.lstatSync(file); } catch (err) { if (err.code !== 'ENOENT') throw err; }
        if (stat && (!stat.isFile() || !fs.readFileSync(file, 'utf8').startsWith('#!/usr/bin/env node\n' + SHIM_MARKER + '\n'))) {
            if (includeNew) throw new Error(`Refusing to replace an unrecognized ${id} shim; no harness shims were changed.`);
            skipped.push(id);
            continue;
        }
        if (!stat && !includeNew) continue;
        ...
        planned.push({id, file, source});
    }
    if (planned.length) fs.mkdirSync(dir, {recursive: true});
    for (const {file, source} of planned) {
        fs.writeFileSync(file, source, {mode: 0o755});
    }
    return {installed: planned.map(p => p.id), skipped};
}
