...
function _realHome() {
    // TEST-ONLY escape hatch (LED-5658 review): a CLI-level integration test
    // needs to reproduce "DELIMIT_HOME exported as the real passwd-home
    // default" WITHOUT ever risking a write to the actual real home (e.g.
    // /root on the dev box this was audited on) if the guard below has a
    // regression. _DELIMIT_TEST_ONLY_REAL_HOME lets a test substitute a
    // disposable tmp dir for "the real home" so a pre-fix/mutated resolver
    // leaks into that disposable dir instead of real production state.
    // Never read by delimit-setup.js, the MCP server, or any non-test code
    // path; a stray production export of this var only affects the
    // mismatch heuristic itself, never file permissions or write targets
    // directly — worst case it makes the guard compare against the wrong
    // "default", which just falls back to trusting $DELIMIT_HOME as-is.
    const testOverride = process.env._DELIMIT_TEST_ONLY_REAL_HOME;
    if (testOverride && testOverride.trim()) {
        return testOverride.trim();
    }
