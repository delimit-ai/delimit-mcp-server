...
echo "  === Delimit Status ==="
# Preserve the pre-existing, read-only startup diagnostics while delegating
# continuity to Session Phoenix. These checks are intentionally before the
# final continuity delivery/acknowledgement phase.
...
[ -f "$DELIMIT_HOME/models.json" ] && MODELS=\$(python3 -c "import json; d=json.load(open('$DELIMIT_HOME/models.json')); print(' + '.join(v.get('name',k) for k,v in d.items() if isinstance(v,dict) and v.get('enabled')))" 2>/dev/null) || true
[ -n "$MODELS" ] && echo "Deliberation: \${MODELS% + }"

# Preserve the rolling session index for operators. This is metadata only;
# actual continuity selection remains project-bound in Session Phoenix below.
...
# Preserve the receiving-agent handoff post-flight. It is read-only,
# time-boxed, and surfaces only critical repository corruption.
...
def _resolve_venture(cwd):
    ...
    # maintained by the CLI resolver (<active namespace>/active_venture.json);
    # fall back to the cwd basename; "all" only when nothing resolves.
    base = os.path.basename(cwd.rstrip("/")) or ""
    home = os.path.expanduser("~")
    delimit_root = (
        os.environ.get("DELIMIT_HOME")
        or os.environ.get("DELIMIT_NAMESPACE_ROOT")
        or os.path.join(home, ".delimit")
    )
    neutral = (
        not base
    ...
    if neutral:
        try:
            with open(os.path.join(delimit_root, "active_venture.json")) as fh:
                reg = json.load(fh)
