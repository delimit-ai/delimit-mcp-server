...
`);
        ...
        // the installed server (~/.delimit/server), picks the GLOBALLY
        // most-recent soul, orchestrator-only, time-boxed (6s), fail-open.
        // Gated by the session_auto_revive config flag (default on; additive +
        // reversible). No JS interpolation lives inside the RVEOF heredoc — the
        // python f-string uses bare {origin} (no dollar-brace), so it survives
        // into the generated script verbatim.
...
if [ "$DELIMIT_SESSION_TYPE" != "subagent" ] && [ "$DELIMIT_SESSION_TYPE" != "agent" ]; then
  DELIMIT_HOME="$DELIMIT_HOME" timeout 6 python3 - <<'RVEOF' 2>/dev/null || true
import os, sys
from pathlib import Path
HOME = Path(os.environ.get("DELIMIT_HOME", str(Path.home() / ".delimit")))
...
try:
    cwd = os.getcwd()
    here = get_latest_soul(cwd)
    glob_ = find_most_recent_soul_across_projects()
    other = glob_.get("soul") if glob_ else None
    other_path = (glob_ or {}).get("project_path", "")
    # Prefer whichever soul is the most recent working state, so a quota-driven
    # agent switch resumes the LAST task even if it happened in a worktree/subdir
    # keyed to a different soul bucket than the relaunch cwd.
    soul, origin = here, cwd
    if other is not None and (here is None or (other.created_at or "") > (here.created_at or "")):
    ...
    if origin and os.path.realpath(origin) != os.path.realpath(cwd):
        print(f"  (most-recent soul, captured under {origin} — revived without cd)")
